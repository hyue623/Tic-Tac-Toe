...
public class TicTacToeModelTest {
  ...
  @Test
  public void testBoardIsFullHaveWinner() {
    ttt1.move(0,0);// X takes top left corner.
    assertFalse(ttt1.isGameOver());
    ttt1.move(0,1); // O takes top middle.
    ttt1.move(0,2); // X takes top right corner.
    assertFalse(ttt1.isGameOver());
    assertNull(ttt1.getWinner());
    ttt1.move(1,0); // O takes middle left.
    ttt1.move(1,1); // X takes middle.
    ttt1.move(1,2); // O takes middle right.
    assertFalse(ttt1.isGameOver());
    assertNull(ttt1.getWinner());
    ttt1.move(2,1); // X takes bottom middle.
    ttt1.move(2,0); // O takes bottom left
    ttt1.move(2,2); // X takes middle right.
    assertTrue(ttt1.isGameOver());
    assertEquals(Player.X, ttt1.getWinner());
  }

  @Test(expected = IllegalArgumentException.class)
  public void testIllegalArgumentOnGetMarkAtRow() {
    ttt1.getMarkAt(8, 0);
  }

  @Test(expected = IllegalArgumentException.class)
  public void testIllegalArgumentOnGetMartAtColumn() {
    ttt1.getMarkAt(3, -3);
  }

  @Test
  public void testGameStartAtPlayerX() {
    assertEquals(Player.X, ttt1.getTurn());
  }

  @Test
  public void testChangeTurnWhenMove() {
    assertEquals(Player.X, ttt1.getTurn());
    ttt1.move(1, 0);
    assertEquals(Player.O, ttt1.getTurn());
  }
  ...
  @Test
  public void testIllegalArgumentMoveOnInvalidRowAndColumn() {
    try {
      ttt1.move(-1, 0);
  ...
  @Test(expected = IllegalStateException.class)
  public void testGameOverCanNotMove() {
    ttt1.move(0, 0);
    ttt1.move(0, 1);
    ttt1.move(1, 0);
    ttt1.move(0, 2);
